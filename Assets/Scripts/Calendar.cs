...
public class Calendar : MonoBehaviour
{
    ...
    public class Day
    {
        ...
        /// Constructor of Day
        /// </summary>
        public Day(int dayNum, Color dayColor, GameObject obj, DateTime currentDate)
        {
            this.dayNum = dayNum;
            this.obj = obj;
        ...
        public void UpdateDay(int newDayNum)
        {
            ...
            if(dayColor == Color.white || dayColor == Color.green)
            {
                ...
                obj.GetComponentInChildren<DayButton>().currentDate = currentDate;
            }
            else
            {
            ...
            for (int w = 0; w < 6; w++)
            {
                for (int i = 0; i < 7; i++)
                {
                    ...
                    if (currDay < startDay || currDay - startDay >= endDay)
                    {
                        ...
                        newDay = new Day(currDay - startDay, Color.grey,weeks[w].GetChild(i).gameObject, getDateTime(year, month, currDay - startDay));
                    }
                    else
                    {
                        newDay = new Day(currDay - startDay, Color.white,weeks[w].GetChild(i).gameObject, getDateTime(year, month, currDay - startDay));
                    }
                    days.Add(newDay);
                }
            }
        ...
        else
        {
            for(int i = 0; i < 42; i++)
            {
                if(i < startDay || i - startDay >= endDay)
                {
                    ...
                    days[i].UpdateDate(getDateTime(year, month, i - startDay));
                }
                else
                {
    ...
    public DateTime getDateTime(int year, int month, int day) {
        ...
        if ((day+1) > GetTotalNumberOfDays(year, month)) {
            return new DateTime(year, 1, 1);
        }
